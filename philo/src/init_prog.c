...
/*   Updated: 2025/10/26 23:21:05 by asando           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_start_suthread(t_data *data, t_philo *philo)
{
	int	i;
	...
	if (data->n_philo > 1)
	{
		if (pthread_create(&(data->super_thread), NULL, super_action,
					 (void *)data)
		{
			ft_system_fail(THREAD_FAIL, "data->super_thread");
			while (i < n_philo)
...
static int	ft_start_thread(t_data *data, t_philo *philo)
{
	...
	while (i < data->n_philo)
	{
		...
		if (pthread_create(&((philo + i)->thread), NULL, philo_action,
					 (void *)(philo + i)))
		{
			ft_system_fail(THREAD_FAIL, "philo->thread");
			while (--i >= 0)
				pthread_join((philo[i])->thread, NULL);
			return (-1);
		}
	...
	if (ft_start_suthread(data, philo))
		return (-1);
	return (0);
}
